...
select
request_at,
round(sum(case when Status = "completed" then 0 else 1)/count(trip.id),2) as cancellation_rate
from trip tr
inner join
(select users_id from users where banned = "No" and Role = "client") users
on tr.client_id = users.users_id
where tr.request_at >= '2013-10-01' and tr.request_at <= '2013-10-03'
